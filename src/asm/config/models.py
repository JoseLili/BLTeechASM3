...
class DisplayConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.idle_notice_seconds <= 10:
            raise ValueError("idle_notice_seconds must be between zero and 10")
        if not 1 <= self.rwt_notice_seconds <= 30:
            raise ValueError("rwt_notice_seconds must be between 1 and 30")
        if not 1 <= self.rwt_summary_seconds <= 30:
            raise ValueError("rwt_summary_seconds must be between 1 and 30")
